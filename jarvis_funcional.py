...
class AzulGUI(ctk.CTk):
    def __init__(self):
        ...
        self.title(f"{nombre_asistente.upper()} v3.0 - Adaptive Intelligence")
        ...
        # --- NUEVOS MÓDULOS DE CALENDARIO ---
        self.gestor_calendario = GestorCalendario(archivo_datos="data/calendario.json")
        self.analizador_calendario = AnalizadorCalendario(modelo='llama3')
...
Recuerda: Conoce al usuario, pero no lo acoses con sus gustos. Usa tu conocimiento para adaptar tu tono y respuestas."""
    ...
    def aprendizaje_en_tiempo_real(self, texto):
        """Usa Llama3 para extraer gustos o hábitos del texto del usuario."""
        # Prompt optimizado - más corto y directo
        prompt = f"Extrae gustos/hábitos como 'clave:valor'. Si no hay, responde 'NADA'. Texto: '{texto}'"
        
        # Opciones para respuesta más rápida
        opciones = {
            'temperature': 0.3,  # Más determinista = más rápido
            'num_predict': 50,   # Pocas palabras necesarias
        }
        
        res = ollama.generate(model='llama3', prompt=prompt, options=opciones)
        insight = res['response'].strip()
        
        if "NADA" not in insight.upper():
            # Guardar en Supabase (background)
            partes = insight.split(":")
            if len(partes) == 2:
    ...
    def obtener_respuesta_ia(self):
        ...
        # Opciones optimizadas para respuestas más rápidas
        opciones = {
            'temperature': 0.7,  # Menos aleatorio = más rápido
            'top_p': 0.9,
            'num_predict': 150,  # Limitar tokens para respuestas cortas
        }
        
        stream = ollama.chat(model='llama3', messages=self.historial, stream=True, options=opciones)
        resp_comp, frase_acum = "", ""
        for chunk in stream:
            parte = chunk['message']['content']
            resp_comp += parte
            frase_acum += parte
            if any(s in parte for s in ['.', '!', '?', '\n']):
                if frase_acum.strip(): cola_voz.put(frase_acum.strip())
                frase_acum = ""
        if frase_acum.strip(): cola_voz.put(frase_acum.strip())
        ...
        threading.Thread(target=lambda: supabase.table("mensajes_chat").insert(
            {"role": "assistant", "content": resp_comp}
        ).execute(), daemon=True).start()
